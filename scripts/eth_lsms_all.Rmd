...
fert_joined  <- st_join(eth_zone_sf, fert_sf, left=FALSE) 
fert_joined
```

```{r}
# reordering columns
fert_df <- fert_joined %>% select(country, start_yr, end_yr, household_id, holder_id, parcel_id, field_id, NAME_1, NAME_2, fert, urea, dap, manure, compost, organic_fert, quant_urea, quant_dap, area) %>% mutate(NAME_1=as.character(NAME_1), NAME_2=as.character(NAME_2))
fert_df
```
...
```{r}
fert_df %>% mutate(usage_urea = quant_urea/area, usage_dap = quant_dap/area, usage_total = (quant_urea + quant_dap) / area)
```
...
```{r}
eth_lsms <- eth_lsms %>% rename(fert_rep_yn = fert, urea_yn = urea, dap_yn = dap, manure_yn = manure, compost_yn = compost, ofert_rep_yn = organic_fert, ofert_yn = organic_fert_dummy, iofert_yn = inorganic_fert_dummy, fert_yn = fert_dummy)
eth_lsms
```
