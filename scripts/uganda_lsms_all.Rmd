...
Also, wave 4 has no location data, so we will attempt to link previously surveyed households together. New households will not be included, since they cannot be geolocated for now (although it might be possible via region data). Past household ids linking wave 4 to wave3 exists in GSEC1, so it is possible to link households that way. However it requires a bunch of regex cleaning, so we will skip this for now.
...
w4_gsec <- read_csv('../lsms_data/uganda/w13/4_GSEC1.csv')
```
...
```{r}
uga<- uga_final %>% select(country, start_yr, end_yr, hhid, plotid, parcelid, ofert_rep_yn, quant_ofert, iofert_rep_yn, type_iofert, quant_iofert, pest_yn, quant_pest, nitrate_quant, phosphate_quant, potash_quant, mixed_quant, nitrate_yn, phosphate_yn, potash_yn, mixed_yn, area, longitude, latitude) %>% rename(quant_nitrate = nitrate_quant, quant_phosphate = phosphate_quant, quant_potash = potash_quant, quant_mixed = mixed_quant)
uga
```
