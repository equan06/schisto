...
Note: in wave3 organic fert is coded differently, but since we don't really care about it (and it is unclear what type is used) we will omit it. 
...
fert3 <- fert_w3 %>% select(zone, state, hhid, plotid, area, fert_yn = s11dq1, 
                            ...
                            bought2_yn = s11dq24, bought2_type = s11dq27, bought2_quant = s11dq28a, bought2_quant_units = s11dq28b) %>% drop_na(fert_yn) 
...
```{r}
t<-fert3_joined
t[t$bought1_quant > 0 & !is.na(t$bought1_quant), ]
```
...
```{r}
fert1 <- fert %>% rename(fert_rep_yn = fert_yn) %>% mutate(ofert_yn = comp_manure_yn, iofert_yn = as.numeric(select(., matches('npk_yn|urea_yn')) %>% rowSums(na.rm = TRUE) > 0)) %>% mutate( fert_yn = as.numeric(select(., matches('ofert_yn|iofert_yn|other_yn')) %>% rowSums(na.rm=TRUE) > 0))
fert1
```

```{r}
fert_df <- fert1 %>% select(country, start_yr, end_yr, zone, state, hhid, plotid, area, fert_rep_yn, npk_yn, urea_yn, comp_manure_yn, other_yn, ofert_yn, iofert_yn, npk_quant, urea_quant, comp_manure_quant, other_quant, longitude, latitude)
fert_df
```
