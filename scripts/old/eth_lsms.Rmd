...
zone_names <- unique(zone_names)
zone_names
```

```{r}
eth_renamed <- merge(eth_final, zone_names)
eth_renamed
```
...
```{r}
schisto <- read_csv('../results/eth_schisto.csv')
schisto
unique(inner_join(schisto, eth_renamed, by = c('zone' = 'zonename', 'year' = 'start_yr')))
```
