...
unique(wave1[wave1$region==12,]['fert']) # wave1 region 12 has 0 fertilizer use.
...
now we compute the p/h/f data and merge it with the fert data on the household level. doing this we lose about 30 observations due to missing data. 

```{r}
w1_phf <- sect4_pp_w1 %>% select(household_id, region=saq01, prevention=pp_s4q04, pest=pp_s4q05, herb=pp_s4q06, fung=pp_s4q07) %>% drop_na(prevention) 

# set 2s to 0s
w1_phf[, -c(1,2)][w1_phf[, -c(1,2)]==2] <- 0


# if no prevention used, set all rest of columns to 0
cols <- 3:ncol(w1_phf) # 3 is first relevant column
w1_phf[w1_phf$phf==0, 3:ncol(w1_phf)] <- 0

w1_phf <- w1_phf %>% group_by(region, household_id) %>% summarize_all(sum, na.rm=TRUE)
w1_phf[, -c(1, 2)] <- ifelse(w1_phf[, -c(1, 2)] > 0, 1, 0)

hh_w1 <- hh_w1 %>% inner_join(w1_phf)
...
p/h/f data
```{r}
w2_phf <- sect4_pp_w2 %>% select(household_id, region=saq01, prevention=pp_s4q04, pest=pp_s4q05, herb=pp_s4q06, fung=pp_s4q07) %>% drop_na(prevention) 

# set 2s to 0s
w2_phf[, -c(1,2)][w2_phf[, -c(1,2)]==2] <- 0


# if no prevention used, set all rest of columns to 0
cols <- 3:ncol(w2_phf) # 3 is first relevant column
w2_phf[w2_phf$phf==0, 3:ncol(w2_phf)] <- 0

w2_phf <- w2_phf %>% group_by(region, household_id) %>% summarize_all(sum, na.rm=TRUE)
w2_phf[, -c(1, 2)] <- ifelse(w2_phf[, -c(1, 2)] > 0, 1, 0)

hh_w2 <- hh_w2 %>% inner_join(w2_phf)
```

```{r}
w2_data <- hh_w2 %>% select(-household_id) %>% 
  group_by(region) %>% summarize_all(mean, na.rm=TRUE)
...
```{r}
w3_phf <- sect4_pp_w3 %>% select(household_id, region=saq01, prevention=pp_s4q04, pest=pp_s4q05, herb=pp_s4q06, fung=pp_s4q07) %>% drop_na(prevention) 

# set 2s to 0s
w3_phf[, -c(1,2)][w3_phf[, -c(1,2)]==2] <- 0


# if no prevention used, set all rest of columns to 0
cols <- 3:ncol(w3_phf) # 3 is first relevant column
w3_phf[w3_phf$phf==0, 3:ncol(w3_phf)] <- 0

w3_phf <- w3_phf %>% group_by(region, household_id) %>% summarize_all(sum, na.rm=TRUE)
w3_phf[, -c(1, 2)] <- ifelse(w3_phf[, -c(1, 2)] > 0, 1, 0)

hh_w3 <- hh_w3 %>% inner_join(w3_phf)
